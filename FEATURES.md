...
**Architecture**:
- **A preset never holds a real and a virtual position on the same symbol at once.** `main.py` passes `real_preset` (the preset of the open real position, from `order_executor.get_open_orders()`). The simulator evicts any rank holding that preset with `real_order_took_over`, and `_try_open` refuses to open it — enforced inside `_try_open` so it holds for every caller, including when the rankings shift and that preset reappears at a different rank mid-trade
- **Scoped to the preset, not the symbol.** Every *other* preset on that symbol keeps collecting, which is the point of the rank pools — a symbol-wide block would stop an actively-trading symbol producing any comparison data at all. A real order on one symbol never touches another symbol's pools
- Rank 1 stands in for the real-order slot, so a signal blocked by a filter still records an outcome. It does not open while the slot is busy (`real_slot_busy` = `_placed_this_candle` OR `get_state(sym) != IDLE`), and it is the one rank that still evicts on a preset change, because it must represent whatever would trade now
- History (2026-09-08): the gate was candle-scoped, so from the next candle onwards the stand-in ran alongside a live real trade — SOLUSDT held a real `l2_trend_buy` at 102.97 and a rank-1 virtual `l2_trend_buy` at 103.63 simultaneously, and 63 more virtual positions on other presets
...
- **Clamping (2026-09-07) — OPT-IN, currently OFF everywhere**: `max_sl_pct` can **clamp the stop to the cap instead of rejecting the signal**, mirroring the `min_sl_pct` floor which has always widened rather than rejected. Implemented once in `config/settings.py:clamp_sl_to_max()` and used by all three engines — `main.py` (live), `bot/virtual_order_simulator.py` (virtual) and `bot/backtester.py`. They must agree: while they disagreed, 107 rejected TIAUSDT signals were invisible in the virtual statistics, so there was no evidence either way for two months.
  ...
  - Logged as `SL clamped: X% -> Y%` and recorded in the decision log as `clamp_max_sl_pct`, so the effect is measurable.
  - **The cap now means "maximum risk per trade", not "reject above this".** Sizing is notional (`quantity = trade_margin * leverage * 1.02 / entry`), so a stop-out at the cap loses that percentage of notional regardless of stop width. At a 12% cap and ~2000 USDT notional that is ~240 USDT, about 7.7% of a 3100 balance, versus ~5.1% at 8%. Raising the cap therefore raises worst-case per-trade loss for the clamped subset, while also admitting trades that previously did not happen at all.
  - **Safer than the old behaviour at the extreme end**: the worst trade in 410 real orders was a SELL with a 22.52% stop (-206.12). Under a 12% clamp that same signal risks 12%.
